...
if [ ! "${GW_API_NUM}" -eq "${api_gw_NUMBER}" ]; then
    ...
    log_warn "Not all API gateway nodes are correctly initialized. Retrying"

    for i in `seq 1 9`; do
        GW_API_NUM=`get_node_number_by_name gateway-api-`
        if [ "${GW_API_NUM}" -eq "${api_gw_NUMBER}" ]; then
            log_ok "All API gateway nodes are now correctly initialized."
            break;
        else
            if [ $i -eq 9 ]; then
                log_err "Not all API gateway nodes are correctly initialized. Exiting."
                exit 3
            else
                log_warn "Attempt $i."
                sleep 20
...
if [ ! "${GW_MESH_NUM}" -eq "${mesh_gw_NUMBER}" ]; then
    ...
    log_warn "Not all Mesh gateway nodes are correctly initialized. Retrying"

    for i in `seq 1 9`; do
        GW_MESH_NUM=`get_node_number_by_name gateway-mesh-`
        if [ "${GW_MESH_NUM}" -eq "${mesh_gw_NUMBER}" ]; then
            log_ok "All Mesh gateway nodes are now correctly initialized."
            break;
        else
            if [ $i -eq 9 ]; then
                log_err "Not all Mesh gateway nodes are correctly initialized. Exiting."
                exit 4
            else
                log_warn "Attempt $i."
                sleep 20
...
if [ ! "${GW_TERM_NUM}" -eq "${term_gw_NUMBER}" ]; then
    ...
    log_warn "Not all Terminating gateway nodes are correctly initialized. Retrying"

    for i in `seq 1 9`; do
        GW_TERM_NUM=`get_node_number_by_name gateway-terminating-`
        if [ "${GW_TERM_NUM}" -eq "${term_gw_NUMBER}" ]; then
            log_ok "All Terminating gateway nodes are now correctly initialized."
            break;
        else
            if [ $i -eq 9 ]; then
                log_err "Not all Terminating gateway nodes are correctly initialized. Exiting."
                exit 5
            else
                log_warn "Attempt $i."
                sleep 20
...
if [ ! "${ESM_NUM}" -eq "${consul_esm_NUMBER}" ]; then
    ...
    log_warn "Not all Consul ESM nodes are correctly initialized. Retrying"

    for i in `seq 1 9`; do
        ESM_NUM=`get_node_number_by_name consul-esm-`
        if [ "${ESM_NUM}" -eq "${consul_esm_NUMBER}" ]; then
            log_ok "All Consul ESM nodes are now correctly initialized."
            break;
        else
            if [ $i -eq 9 ]; then
                log_err "Not all Consul ESM nodes are correctly initialized. Exiting."
                exit 6
            else
                log_warn "Attempt $i."
                sleep 20
...
if [ ! "${DB_NUM}" -eq "${hashicups_db_NUMBER}" ]; then
    ...
    log_warn "Not all HashiCups DB nodes are correctly initialized. Retrying"

    for i in `seq 1 9`; do
        DB_NUM=`get_node_number_by_name hashicups-db-`
        if [ "${DB_NUM}" -eq "${hashicups_db_NUMBER}" ]; then
            log_ok "All HashiCups DB nodes are now correctly initialized."
            break;
        else
            if [ $i -eq 9 ]; then
                log_err "Not all HashiCups DB nodes are correctly initialized. Exiting."
                exit 7
            else
                log_warn "Attempt $i."
                sleep 20
...
if [ ! "${API_NUM}" -eq "${hashicups_api_NUMBER}" ]; then
    ...
    log_warn "Not all HashiCups API nodes are correctly initialized. Retrying"

    for i in `seq 1 9`; do
        API_NUM=`get_node_number_by_name hashicups-api-`
        if [ "${API_NUM}" -eq "${hashicups_api_NUMBER}" ]; then
            log_ok "All HashiCups API nodes are now correctly initialized."
            break;
        else
            if [ $i -eq 9 ]; then
                log_err "Not all HashiCups API nodes are correctly initialized. Exiting."
                exit 8
            else
                log_warn "Attempt $i."
                sleep 20
...
if [ ! "${FE_NUM}" -eq "${hashicups_frontend_NUMBER}" ]; then
    ...
    for i in `seq 1 9`; do
        FE_NUM=`get_node_number_by_name hashicups-frontend-`
        if [ "${FE_NUM}" -eq "${hashicups_frontend_NUMBER}" ]; then
            log_ok "All HashiCups Frontend nodes are now correctly initialized."
            break;
        else
            if [ $i -eq 9 ]; then
                log_err "Not all HashiCups Frontend nodes are correctly initialized. Exiting."
                exit 9
            else
                log_warn "Attempt $i."
                sleep 20
...
if [ ! "${LB_NUM}" -eq "${hashicups_nginx_NUMBER}" ]; then
    ...
    log_warn "Not all HashiCups NGINX nodes are correctly initialized. Retrying"

    for i in `seq 1 9`; do
        LB_NUM=`get_node_number_by_name hashicups-nginx-`
        if [ "${LB_NUM}" -eq "${hashicups_nginx_NUMBER}" ]; then
            log_ok "All HashiCups NGINX nodes are now correctly initialized."
            break;
        else
            if [ $i -eq 9 ]; then
                log_err "Not all HashiCups NGINX nodes are correctly initialized. Exiting."
                exit 7
            else
                log_warn "Attempt $i."
                sleep 20
