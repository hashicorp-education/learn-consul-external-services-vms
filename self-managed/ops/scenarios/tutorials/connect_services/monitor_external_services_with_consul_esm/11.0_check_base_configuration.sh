...
header1 "Scenario: Manage permissions with Access Control Lists (ACLs)"
...
_hashicups_api_MIN=1
_hashicups_api_MAX=1
...
_check_value_in_range "consul-esm" "${consul_esm_NUMBER}" "${_consul_esm_MIN}" "${_consul_esm_MAX}"
...
if [ "${ENABLE_SERVICE_MESH}" != "false" ]; then
    log_err "Service mesh enabled. Scenario cannot be demonstrated."
    exit 1
fi
