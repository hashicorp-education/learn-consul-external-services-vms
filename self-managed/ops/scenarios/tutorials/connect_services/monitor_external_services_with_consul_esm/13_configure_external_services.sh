...
if [ "${consul_esm_NUMBER}" -gt "0" ]; then
  ...
  ## At this point we expect to have a consul-esm binary into /home/${username}/bin
  BIN_FILE=`find /home/${_USER}/bin | grep consul-esm | sort -r -V | head -1`
  ...
  if [ -f "${BIN_FILE}" ]; then
    ...
    for i in `seq ${consul_esm_NUMBER}`; do
      ...
      remote_copy "${NODE_NAME}" "${BIN_FILE}" "/home/${_USER}/consul-esm"

      remote_exec ${NODE_NAME} "chmod +x /home/${_USER}/consul-esm && sudo mv /home/${_USER}/consul-esm /usr/bin/consul-esm"
